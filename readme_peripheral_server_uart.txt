...
This sample project passes through several states before all services are enabled:
...
The application needs to be connected to a terminal application or similar 
that can read and write serial UART data at 115200 baud. For a QFN-based RSL10 
Evaluation and Development Board, no external connections are required. For a
WLCSP-based RSL10 Evaluation and Development Board, connect:
...
"APPM_ADVERTISING", the LED on the Evaluation and Development Board (DIO6) is 
blinking. It turns on steadily once the link is established, and it enters the
